            ...
            while let Some(fre) = iter.next()? {
                ...
                let rest = match self.abi {
                    SFrameABI::AMD64LittleEndian => {
                        // https://sourceware.org/binutils/docs-2.46/sframe-spec.html#AMD64
                        let ra = "f"; // fixed
                        format!("{cfa:8} {fp:6} {ra}")
                    }
                    SFrameABI::AArch64LittleEndian => {
                        // https://sourceware.org/binutils/docs-2.46/sframe-spec.html#AArch64
...
impl SFrameFDE {
    ...
    pub fn get_pc(&self, section: &SFrameSection<'_>) -> u64 {
        ...
        {
            (self.func_start_offset as i64)
                .wrapping_add_unsigned(self.offset as u64)
                .wrapping_add_unsigned(section.section_base) as u64
        } else {
            (self.func_start_offset as i64).wrapping_add_unsigned(section.section_base) as u64
        }
    }
