...
impl<'a> SFrameSection<'a> {
    ...
    pub fn get_fde(&self, index: u32) -> SFrameResult<Option<SFrameFDE>> {
        ...
        Ok(Some(SFrameFDE {
            ...
            func_rep_size: self.data
                [offset + core::mem::offset_of!(RawSFrameFDE, sfde_func_rep_size)],
        }))
    }
...
/// Raw SFrame Preamble
///
/// Ref: <https://sourceware.org/binutils/docs-2.45/sframe-spec.html#SFrame-Preamble>
#[repr(C, packed)]
...
struct RawSFrameFDE {
    ...
    sfde_func_info: u8,
    sfde_func_rep_size: u8,
    sfde_func_padding2: u16,
}
...
impl SFrameFREInfo {
    ...
    pub fn get_offset_count(&self) -> u8 {
        // A max value of 15 is allowed. Typically, a value of upto 3 is
        // sufficient for most ABIs to track all three of CFA, FP and RA.
        (self.0 >> 1) & 0b1111
    }
